import { trackPromosPaginaVista, trackPromoClick } from '../utils/analytics';
...
import {
  Percent, Clock, RefreshCw, AlertCircle, ChevronRight,
  Fuel, Calendar, BadgePercent, Info,
} from 'lucide-react';
...
const BANCO_COLORS: Record<string, { bg: string; border: string; badge: string; text: string }> = {
  ...
  'Galicia':       { bg: 'bg-red-950/60',    border: 'border-red-500/50',    badge: 'bg-red-700',    text: 'text-red-300' },
  'BBVA':          { bg: 'bg-blue-950/60',   border: 'border-blue-500/50',   badge: 'bg-blue-600',   text: 'text-blue-300' },
  'Naranja X':     { bg: 'bg-orange-950/60', border: 'border-orange-500/50', badge: 'bg-orange-600', text: 'text-orange-300' },
  'Ualá':          { bg: 'bg-purple-950/60', border: 'border-purple-500/50', badge: 'bg-purple-600', text: 'text-purple-300' },
};
const DEFAULT_COLOR = { bg: 'bg-slate-800/60', border: 'border-slate-600/50', badge: 'bg-slate-600', text: 'text-slate-300' };

const MARCA_LOGO: Record<string, string> = {
  'YPF':   '🔵', 'Shell': '🔴', 'Axion': '🟠',
  'Puma':  '🟣', 'Gulf':  '🟡', 'Todas': '⛽',
};
...
function formatDate(dt: string | null): string {
  if (!dt) return '';
  try {
    return new Date(dt + 'Z').toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' });
  } catch { return dt.split('T')[0]; }
}

// ── Card de promo ──────────────────────────────────────────────────────────────
function PromoCard({ promo }: { promo: Promo }) {
  const c        = BANCO_COLORS[promo.banco] || DEFAULT_COLOR;
  const emoji    = MARCA_LOGO[promo.marca] || '⛽';
  ...
  return (
    <div className={`rounded-2xl border ${c.border} ${c.bg} p-5 flex flex-col gap-3 backdrop-blur-sm hover:scale-[1.01] transition-transform`}>
      ...
      {/* Nombre del banco si hay logo */}
      {promo.logo_url && (
        <span className={`text-xs font-bold px-2 py-0.5 rounded-full text-white self-start ${c.badge}`}>
          {promo.banco}
        </span>
      )}
      ...
      <div>
        <div className="flex items-baseline gap-1">
          ...
          <span className="text-slate-400 text-sm font-medium">de reintegro<br/>en combustible</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1.5 text-sm border-t border-slate-700/50 pt-3">
        ...
        {promo.tope && (
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <span>Tope: <strong className="text-white">{promo.tope}</strong></span>
          </div>
        )}
        {promo.vigencia && (
          <div className="flex items-center gap-2 text-slate-300">
        ...
        {promo.condiciones && (
          <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">{promo.condiciones}</p>
        )}
      </div>

      {/* Fuente */}
      {promo.fuente_url && promo.fuente_nombre && (
        <div className="border-t border-slate-700/30 pt-2 mt-auto">
          <a
            href={promo.fuente_url}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 transition-colors"
          >
            <Info size={11} />
            Fuente: {promo.fuente_nombre} ↗
          </a>
        </div>
      )}
    </div>
...
export function PromosPage() {
  ...
  useEffect(() => { cargar(); trackPromosPaginaVista(); }, []);

  const ahora  = new Date();
  const mesAño = ahora.toLocaleDateString('es-AR', { month: 'long', year: 'numeric' });
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-0.5">
                Promos vigentes — {mesAño}
              </p>
            </div>
          </div>

          <p className="text-slate-300 mt-4 max-w-2xl text-sm leading-relaxed">
            Reintegros y descuentos disponibles al cargar nafta, gasoil o GNC en Argentina.
            Se actualizan automáticamente el <strong className="text-white">1° y 15 de cada mes</strong>.
            Combiná bank y día para maximizar tu ahorro.
          </p>

          {data?.scraped_at && (
            <p className="text-xs text-slate-500 mt-3 flex items-center gap-1.5">
              <RefreshCw size={11} />
              Última actualización: {formatDate(data.scraped_at)}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 mt-6">
        <div className="flex items-start gap-2.5 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3 text-xs text-amber-200">
          ...
          <span>
            Los reintegros se acreditan en tu cuenta bancaria o billetera después de cada carga.
            Verificá topes y condiciones en la app de cada banco o estación antes de cargar.
            Las promos pueden cambiar sin previo aviso.
          </span>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="rounded-2xl bg-slate-800/50 animate-pulse h-44" />
            ))}
          </div>
        )}
        ...
        {error && (
          <div className="flex items-center gap-3 bg-red-950/50 border border-red-800 rounded-xl p-4 text-red-300">
            ...
            <button onClick={cargar} className="ml-auto flex items-center gap-1 text-xs underline hover:text-red-200">
          ...
          <>
            <div className="flex items-center justify-between mb-5">
              <p className="text-slate-400 text-sm">
                <strong className="text-white">{data.total}</strong> descuentos vigentes
              </p>
              <button
                onClick={cargar}
                className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-white border border-slate-700 rounded-lg px-3 py-1.5 hover:bg-slate-800 transition-colors"
              >
        ...
        <div className="mt-12 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 rounded-2xl p-6 flex flex-col sm:flex-row items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-slate-400 text-sm mt-1">
              Combiná los descuentos del día con los precios más bajos del mapa.
            </p>
          </div>
          <Link
            to="/"
            className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-black font-bold text-sm px-5 py-3 rounded-xl transition-colors whitespace-nowrap"
          >
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-slate-300">
            ...
            <div className="bg-slate-800/50 rounded-xl p-4">
              ...
              <p>Algunos bancos y apps son acumulables. Usá el mapa de Tankear para encontrar la estación más barata.</p>
            </div>
          </div>
        </div>
      </main>
